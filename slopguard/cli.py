...
def analyze(
    ...
    head: Optional[str] = typer.Option("HEAD", help="Head git ref for diff comparison."),
    ...
    output_format: str = typer.Option("text", help="Output format: text, json, sarif, github"),
):
    """
...
def explain(rule_id: str):
    ...
    if rule_id in registry:
        ...
        if fnd:
            console.print(f"[bold cyan]Category:[/bold cyan] {', '.join(c.value for c in fnd.categories)}")
            console.print(f"[bold red]Default Severity:[/bold red] {fnd.severity.upper()}")
...
def config(action: str = typer.Argument("init", help="init to generate config")):
    ...
    if action == "init":
        ...
        conf.save()
        console.print("[green]Created .slopguard.yml with default configuration.[/green]")

@app.command()
def autofix(safe: bool = typer.Option(True, "--safe", help="Only apply 100% safe fixes.")):
    """
