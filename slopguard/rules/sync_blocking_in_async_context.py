...
class SyncBlockingRule(BaseRule):
    ...
    def evaluate(self, file_path: str, code: str, parsed_ast: Any, parser: BaseParser) -> List[Finding]:
        findings = []
                ...
                if isinstance(node, ast.AsyncFunctionDef):
                    for subnode in ast.walk(node):
                        if isinstance(subnode, ast.Call) and isinstance(subnode.func, ast.Attribute):
                            ...
                            if subnode.func.attr in ("sleep", "get", "post", "put", "delete") and getattr(subnode.func.value, "id", "") in ("time", "requests"):
